...
template<typename T>
static std::string makeString(llvm::ArrayRef<T> array) {
  std::string buf;
  buf.clear();
  llvm::raw_string_ostream os(buf);
  os << "[";
...
static
bool vnniVerifier(size_t axis, llvm::ArrayRef<int64_t> tdescShape, 
                    llvm::ArrayRef<int64_t> valueShape, size_t elemTyBitWidth) {  
...
mlir::LogicalResult CreateNdDescOp::verify() {
  ...
  if (getOffsetsRank() != getShapeRank() || getShapeRank() != getStridesRank() || 
      ...
      return emitOpError("Expecting the rank of shape, strides and offsets should match with each other.");
...
mlir::LogicalResult LoadNDOp::verify() {
  ...
  if (!getVnniAxis()) {
    ...
    if (!vnniVerifier(axis, tdescShape, valueShape, bits)) 
      return emitOpError("Invalid vnni transform. When vnni is enabled, value should have one more"
                         "dimention than the TensorDesc, but having same number of data elements."
                         "Also, vnni factor should be calculated as simd_lane_width / elementTypeBitWidth."
...
::mlir::LogicalResult StoreScatterOp::verify() {
  // length of the offsets vector must match the dim-0 of the tensor descriptor
  if (getTensorDesc().getType().getShape() !=
...
#include <triton/Dialect/XeGPU/IR/XeGPUOpsEnums.cpp.inc>
#define GET_OP_CLASSES
#include <triton/Dialect/XeGPU/IR/XeGPUOps.cpp.inc>
